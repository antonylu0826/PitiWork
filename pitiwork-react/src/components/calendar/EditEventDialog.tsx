    ...
    while (current.isSameOrBefore(end)) {
        ...
        switch (recurrenceType) {
            ...
            case '1': // Weekly
                ...
                break;
            case '2': // Monthly
                if (current.date() === parseInt(monthlyDayNumber, 10)) {
                    isValid = true;
                }
                break;
            case '3': // Yearly
                if (current.month() + 1 === parseInt(yearlyMonth, 10) && current.date() === parseInt(yearlyDayNumber, 10)) {
                    isValid = true;
                }
                break;
        }
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        ...
        <FormControlLabel control={<Checkbox checked={allDay} onChange={(e) => setAllDay(e.target.checked)} color="primary" />} label="All Day Event" />
        ...
        <Collapse in={isRecurrent}>
            ...
            <Collapse in={recurrenceType === '1'} sx={{ pl: 2, mt: 1 }}>
            ...
            <Collapse in={recurrenceType === '2'} sx={{ pl: 2, mt: 1 }}>
                <TextField label="Day of Month" type="number" fullWidth variant="outlined" value={monthlyDayNumber} onChange={(e) => setMonthlyDayNumber(e.target.value)} inputProps={{ min: 1, max: 31 }} />
            </Collapse>

            <Collapse in={recurrenceType === '3'} sx={{ pl: 2, mt: 1 }}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={6}>
                        <TextField label="Day of Month" type="number" fullWidth variant="outlined" value={yearlyDayNumber} onChange={(e) => setYearlyDayNumber(e.target.value)} inputProps={{ min: 1, max: 31 }} />
                    </Grid>
                </Grid>
            </Collapse>

            <TextField margin="dense" label="End Date" type="date" fullWidth variant="outlined" value={recurrenceEndDate} onChange={(e) => setRecurrenceEndDate(e.target.value)} InputLabelProps={{ shrink: true }} />
        </Collapse>
